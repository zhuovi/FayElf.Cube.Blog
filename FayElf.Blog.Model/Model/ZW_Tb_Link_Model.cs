...
namespace FayElf.Blog.Model
{
    ...
    /// </summary>
    [Table(Name = "ZW_Tb_Link", PrimaryKey = "ID", ModelType = ModelType.Table, ConnName = "FayElf.Cube.Blog", ConnIndex = 0)]
    public class ZWTbLinkModel : Entity<ZWTbLinkModel>
	{
        #region 构造器
        /// <summary>
        /// 无参构造器
        /// </summary>
        public ZWTbLinkModel() { }
		#endregion
		...
		[Column(Name = "LinkType", PrimaryKey = false, AutoIncrement = false, DataType = "INTEGER", DefaultValue = "1", Description = "LinkType", IsNullable = false, Length = 1, Digit = 0, IsUnique = false, IsIndex = false, AutoIncrementSeed = 0, AutoIncrementStep = 0)]
        public int? LinkType
        {
            get { return this._LinkType; }
            set
            {
                if (!this._LinkType.EqualsIgnoreCase(value))
                {
                    var val = this._LinkType;
                    this._LinkType = value;
                    this.AddDirty("LinkType", val, value);
                }
            }
        }
        /// <summary>
		...
		/// 析构器
		/// </summary>
		~ZWTbLinkModel() { base.Dispose(true); }
		#endregion
	}
    #endregion
}