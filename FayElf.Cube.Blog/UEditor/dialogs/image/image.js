    ...
    SearchImage.prototype = {
        ...
        getImageData: function(){
            var _this = this,
                ...
                url = "/UEditor/SearchImage?word=" + key + " &keeporiginname=" + keepOriginName + "&" + new Date().getTime();
                //url = "//image.baidu.com/search/acjson?tn=resultjson_com&ipn=rj&ct=201326592&is=&fp=result&cl=2&lm=-1&ie=utf-8&oe=utf-8&adpicid=&st=-1&z=&ic=0&s=&se=&tab=&width=&height=&face=0&istype=2&qc=&nc=1&fr=&pn=100&rn=100&gsm=1e00001e&queryWord=" + encodeURIComponent(key) + /*type +*/ "&word=" + key + "&keeporiginname=" + keepOriginName + "&" + new Date().getTime();
                    ...
                    if (json && json.data) {
                        ...
                        for(var i = 0; i < json.data.length; i++) {
                            if (json.data[i].objURL) {
                                list.push({
                                    title: json.data[i].fromPageTitle,
                                    src: json.data[i].thumbURL,
                                    url: json.data[i].thumbURL,
                                    alt: json.data[i].fromPageTitleEnc
                                });
        ...
        setList: function (list) {
            ...
            if (list.length) {
                ...
                for (i = 0; i < list.length; i++) {
                    ...
                    img.setAttribute('title', list[i].alt);
                    img.onerror = function () {
                        this.parentNode.parentNode.style = 'display:none;';
                    }
                    link.href = list[i].url;
                    link.target = '_blank';
                    link.title = list[i].alt;
